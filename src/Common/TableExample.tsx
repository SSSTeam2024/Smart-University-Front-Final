    ...
    <View style={{ margin: 10 }}>
      {elements.map((element, index) => {
        if (element.nodeType === 1 && element.nodeName === 'P') {
          let paragraphText = element.textContent?.replace(/\u00A0/g, ' ') || '';
          let nextElement = element.nextSibling;
          while (nextElement && nextElement.nodeType === 1 && nextElement.nodeName === 'P') {
            paragraphText += ` ${nextElement.textContent?.replace(/\u00A0/g, ' ') || ''}`;
          ...
          const rows = Array.from(tableElement.querySelectorAll('tr')).map((row) => {
            const cells = Array.from(row.querySelectorAll('td')).map((cell) => {
              ...
              const cellContent = cell.innerHTML.replace(/<br\s*\/?>/gi, '\n');
              ...
              {rows.map((row, rowIndex) => {
                ...
                >
                  {row.map((cell, cellIndex) => (
                    <Text key={`cell-${cellIndex}`} style={{ width: cellWidth, padding: 5, fontWeight: 'bold' }}>
                      {cell}
                    </Text>
                  ))}
                </View>)
