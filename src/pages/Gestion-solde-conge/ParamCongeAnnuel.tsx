...
import { useUpdateLeaveTypeMutation, useFetchLeaveTypeByIdQuery } from "features/congé/leaveTypeSlice";
...
const EditAnnuelLeaveType = () => {
  ...
  const { data: leaveType, isLoading, isError } = useFetchLeaveTypeByIdQuery({ _id: idLeaveType.id || "" });
  ...
  const handleAddSubcategory = () => {
    ...
    setFormState({ ...formState, subcategories: [...(formState.subcategories || []), newSubcategory] });
  };
  ...
  const handleRemoveSubcategory = (index: number) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const updatedSubcategories = formState.subcategories?.filter((_, i) => i !== index);
        setFormState({ ...formState, subcategories: updatedSubcategories });
  ...
  return (
    ...
    <div className="page-content">
      ...
      <h5>Type de congé: <span>{formState.name_fr} | {formState.name_ar} </span></h5>
 ...
 <Col lg={2}>
          <Form.Group className="mb-2">
            <Form.Label>Nom de la sous-catégorie (français)</Form.Label>
            <Form.Control
              type="text"
              value={subcategory.name_fr}
              onChange={(e) => handleSubcategoryChange(index, "name_fr", e.target.value)}
            />
          </Form.Group>
          </Col>
          ...
          <Col lg={2}>
          <Form.Group className="mb-2">
            <Form.Label>Nom de la sous-catégorie (arabe)</Form.Label>
            <Form.Control
              type="text"
          ...
          <Col lg={2}>
          <Form.Group className="mb-2">
            <Form.Label>Nombre Maximum de Jours</Form.Label>
            <Form.Control
              type="number"
              value={subcategory.maxDays}
              onChange={(e) => handleSubcategoryChange(index, "maxDays", Number(e.target.value))}
            />
          </Form.Group>
          </Col>
          {/* Sex */}
          ...
          <Form.Group className="mb-2">
            ...
            <Form.Control
              ...
              onChange={(e) => handleSubcategoryChange(index, "sexe", e.target.value)}
            >
        ...
        {gradePersonnel?.map((grade) => (
          ...
          <Form.Group className="mb-2">
            ...
            <Form.Check
              ...
              onChange={(e) => handleSubcategoryChange(index, "Accumulable", e.target.checked)}
            />
          </Form.Group>
          </Col>
