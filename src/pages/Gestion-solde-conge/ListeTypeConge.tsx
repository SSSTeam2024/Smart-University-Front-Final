...
import {useFetchLeaveTypeQuery,useAddLeaveTypeMutation, useUpdateLeaveTypeMutation, useDeleteLeaveTypeMutation} from "features/congé/leaveTypeSlice";
...
const ListeLeaveType = () => {
  ...
  const [visibleSubcategories, setVisibleSubcategories] = useState<VisibleSubcategories>({});
    ...
    <React.Fragment>
      ...
      <Row className="align-items-center mb-4">
        ...
        <Col className="text-end">
          <Button onClick={handleNavigate} variant="primary">Ajouter type congé</Button>
        </Col>
      </Row>
            ...
            <Col lg={4} md={6} sm={12} key={leaveType._id} className="mb-4">
              <Card className="h-100 shadow-sm">
                ...
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  <Card.Text>
                    <strong>Accumulable:</strong> {leaveType.Accumulable ? "Oui" : "Non"}<br />
                    <strong>Sexe:</strong> {leaveType.sexe === "Both" ? "Homme et femme" : leaveType.sexe === "Homme" ? "Homme" : "Femme"} <br />
                    <strong>Description:</strong> {leaveType.description || "N/A"}
                  </Card.Text>
                  ...
                  >
                    {visibleSubcategories[leaveType._id] ? "Masquer les sous-catégories" : "Voir les sous-catégories"}
                  </Button>

                  {/* Conditionally render subcategories */}
                  {visibleSubcategories[leaveType._id] && leaveType.subcategories && (
                    <ul className="list-unstyled">
                      {leaveType.subcategories.map((subcategory, index) => (
                        <li key={index}>{subcategory.name_fr}: ({subcategory.maxDays} jours)</li>
                      ))}
                    </ul>
                  )}
