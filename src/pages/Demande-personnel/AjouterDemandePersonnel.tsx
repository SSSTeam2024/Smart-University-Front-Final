...
import NumberSpinner from "Common/ScrollInput";
const AjouterDemandePersonnel = () => {
  ...
  const personnel: Personnel[] = Array.isArray(personnels)
    ? personnels
    : [];
  ...
  const filteredTemplates = templateBody.filter(
    (template) => selectedLangue && // Make sure selectedLangue is not empty
